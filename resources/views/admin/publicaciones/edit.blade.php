...
<div class="card ">
    <div class="card-body">
        @if (Auth::check() && Auth::user()->rol == 'admin')
            ...
            <div class="row">
                <div class="col-12">
                    <form action="{{ route('admin.publicaciones.update',$publicacion->id) }}" method="post" enctype="multipart/form-data" class="col-12">
                        @csrf
                        @method('PUT')
                        @if ($errors->any())
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-12">
                                ...
                                <textarea type="text" class="form-control" id="descripcion" name="descripcion" value="{{ $publicacion->descripcion }}">{{ $publicacion->descripcion }}</textarea>
                            </div>
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                ...
                                <select class="js-example-basic-multiple form-control" id="investigadores"  name="investigadores[]" multiple="multiple">
                                    @foreach ($investigadores as $investigador)
                                        @if ($publicacion->investigadores->contains($investigador->id))
                                            <option selected value="{{$investigador->id}}">{{$investigador->id.'. '.$investigador->nombre.' '.$investigador->apellidos}}</option>
                                        @else
                            ...
                            <div class="col-md-6">
                                ...
                                <input autocomplete="off" type="text" class="date-own form-control" id="anio" name="anio" value="{{ $publicacion->anio }}">
                            </div>
                        </div>
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                ...
                                @foreach($archivos as $archivo)
                                    <div class="file">
                                        <a target="_blank" href="{{url('storage/files/publicaciones/'.$archivo->path)}}">{{$archivo->path}}</a><a href="{{ route('publicacion-delete-archivo',$archivo->id) }}" class="delete-file btn btn-danger m-2">x</a>
                                    </div>
